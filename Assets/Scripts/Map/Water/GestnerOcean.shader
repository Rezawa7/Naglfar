Shader "Custom/GerstnerOcean"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 _Dir1; float _Amp1,_Wave1,_Steep1,_Phase1;
            float2 _Dir2; float _Amp2,_Wave2,_Steep2,_Phase2;
            float2 _Dir3; float _Amp3,_Wave3,_Steep3,_Phase3;
            float2 _Dir4; float _Amp4,_Wave4,_Steep4,_Phase4;
            ...
            void Gerstner(
                float3 worldPos,
                float2 dir, float A, float L, float S, float phaseOff,
                float time,
                inout float3 pos,
                inout float3 normal)
            {
                float k  = UNITY_PI * 2.0 / L;
                float Qi = S * A * k;
                float2 d = normalize(dir);
                float phase = dot(worldPos.xz, d) * k
                            - time * k
                            + phaseOff;
                float c = cos(phase);
                float s = sin(phase);
            ...
            v2f vert(appdata v)
            {
                ...
                // Fix: Use unity_WorldToObject instead of UNITY_MATRIX_I_M (inverse model matrix)
